		...
		<form class="form-horizontal well" method="POST" enctype='multipart/form-data' action="<?php echo $this->sendTo; ?>">
			<fieldset>
					...
					<div class="controls">
						<?php if(isset($value["link"])&&$value["link"]){
							...
							switch($value["type"]){
								case "text":
									?>
									<input <?php echo $disabled; ?> type="<?php echo $value["type"]; ?>" name="<?php echo $key; ?>" value="<?php echo $this->validator->getValue($key); ?>" class="input-xlarge text-tip" id="<?php echo $key; ?>" <?php if(isset($value["tooltip"])) echo "title=\"".$value['tooltip']."\""; ?> <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?> />
									...
									<?php // if (isset($value["error"])) { ?>
										<!-- <script>
											var attr_<?php echo $key; ?> = $("form [name='<?php echo $key; ?>']").attr("data-original-title");
											if (typeof attr_<?php echo $key ?> !== 'undefined' || attr_<?php echo $key; ?> != false) {
												$("form [name='<?php echo $key; ?>']").removeAttr("data-original-title");
												$("form [name='<?php echo $key; ?>']").attr("title","<?php echo $value["error"]; ?>");
											}
										</script> -->
									<?php // } ?>
									<?php
								...
								case "multiple":
									?>
									<div class="multiple">
										<input <?php echo $disabled; ?> type="<?php echo $value["multiple"]; ?>" name="<?php echo $key; ?>[1]" value="<?php echo $this->validator->getValue($key."1"); ?>" class="input-xlarge text-tip" id="<?php echo $key; ?>1" <?php if(isset($value["tooltip"])) echo "title=\"".$value['tooltip']."\""; ?> <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?> />
									</div>
								...
								case "date":
									?>
									<input <?php echo $disabled; ?> class="datepicker" type="text" name="<?php echo $key; ?>" value="<?php echo $this->validator->getValue($key); ?>" class="input-xlarge text-tip" id="<?php echo $key; ?>" <?php if(isset($value["tooltip"])) echo "title=\"".$value['tooltip']."\""; ?> <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?> />
									<?php if(isset($value["linkto"])){ ?>
										<script>
											$(function(){
												$("form [name='<?php echo $value["linkto"]; ?>']").change(function(){
								...
								case "textarea":
									?>
									<textarea <?php echo $disabled; ?> name="<?php echo $key; ?>" id="<?php echo $key; ?>" <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?>><?php echo $this->validator->getValue($key); ?></textarea>
									<?php if(isset($value["linkto"])){  ?>
										<script>
											$(function(){
												$("form [name='<?php echo $value["linkto"]; ?>']").change(function(){
								...
								case "cooltextarea":
									...
									<textarea <?php echo $disabled; ?> class="cooltextarea" name="<?php echo $key; ?>" id="<?php echo $key; ?>" <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?>><?php echo $this->validator->getValue($key); ?></textarea>
									<script>
										$(function(){
								...
								case "checkbox":
									foreach($value["checkbox"] as $key2=>$value2){
										...
										<input <?php if($value2["id"]==$this->validator->getValue($key)) echo "checked='checked'" ?> <?php echo $disabled; ?> value="<?php echo $key2; ?>" type="checkbox" name="<?php echo $key; ?>" class="input-xlarge text-tip" id="<?php echo $key."_".$key2; ?>" <?php if(isset($value["tooltip"])) echo "title=\"".$value['tooltip']."\""; ?> <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?> />
								...
								case "radio":
									foreach($value["radio"] as $key2=>$value2){
										?>
										<label class="checkbox inline">
										<input <?php if($value2["id"]==$this->validator->getValue($key)) echo "checked='checked'" ?> <?php echo $disabled; ?> value="<?php echo $key2; ?>" type="radio" name="<?php echo $key; ?>" class="input-xlarge text-tip" id="<?php echo $key."_".$key2; ?>" <?php if(isset($value["tooltip"])) echo "title=\"".$value['tooltip']."\""; ?> <?php echo (isset($value["required"]) && !isset($value["showwhenequal"])) ? "required" : ""; ?> />
										<?php echo $value2["name"]; ?>
										</label>
										<?php if (isset($value["required"])) { ?>
											<script>
				...
				<div class="form-actions" >
					...
					<button onsubmit="return false;window.location.replace('<?php echo $this->div_id; ?>');" class="btn btn-success">Siguiente</button>
...
<script>
	...
	function next() {
		...
		foreach ($this->columns as $key => $value) { ?>
			if ($("form [name='<?php echo $key; ?>']").attr('required') && $("form [name='<?php echo $key; ?>']") != "") {
				i++;
			}
			cnt++;
			<?php
		...
		if (i == cnt) {
			$(this).submit();
			window.location.replace('<?php echo $this->div_id; ?>');
			return false;
		}
	}
</script>
