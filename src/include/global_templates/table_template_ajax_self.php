...
<table class="table table-default table-bordered table-color-odd-even">
	<thead>
		<tr>
...
<script>
	function setValue(e,id,key){
		e.preventDefault();
		//t = e;
		$(e.target.parentNode.parentNode).find("#"+key+"_"+id).hide();
		$(e.target.parentNode.parentNode).find("#set_"+key+"_"+id).show();
		//$("#"+key+"_"+id).hide();
		//$("#set_"+key+"_"+id).show();
	}
	function unsetValue(e,id,key){
		...
		//$("#"+key+"_"+id).show();
		//$("#set_"+key+"_"+id).hide();
		$(e.target.parentNode.parentNode.parentNode).find("#"+key+"_"+id).show();
		$(e.target.parentNode.parentNode.parentNode).find("#set_"+key+"_"+id).hide();
	}
	function updateValue_<?php echo $this->bodyId; ?>(e,id,key,index){
		e.preventDefault();
		index = typeof index !== 'undefined' ? index : 'id';
		var val=$(e.target.parentNode.parentNode.parentNode).find("#new_"+key+"_"+id).val();
		$.post('<?php echo $html->link($this->page_name,"ajax").$action."/?".$str_get; ?>', {page:0,ajax:1,action:"update",id:id, str: key, type: index,val:val,table:"<?php echo $this->bodyId; ?>"},
		function(data) {
			$(e.target.parentNode.parentNode.parentNode).find("#"+key+"_"+id).show();
			$(e.target.parentNode.parentNode.parentNode).find("#set_"+key+"_"+id).hide();
			//location.reload();
...
<script>
	function downloadReport_<?php echo $this->bodyId; ?>(e,engine){
		...
		post('<?php echo $html->link($this->page_name,"ajax").$action."/?".$str_get; ?>', {action:engine,ajax:1,type: '', page: 0,
...
<script>
	function delete_<?php echo $this->bodyId; ?>(e,id,str){
		...
		if(confirm("Esta seguro que lo desea borrar?")){
			$.post('<?php echo $html->link($this->page_name,"ajax").$action."/?".$str_get; ?>', { page:0,ajax:1,action:"delete",id:id, str: str, type: 'show_table',key:"show_table",table:"<?php echo $this->bodyId; ?>"},
			function(data) {
...
<script>
	function updateKeyPress(id) {
		if (event.keyCode == 13) {
			$("#"+id).siblings(".accept_co").trigger("onclick");
		} else if (event.keyCode == 27) {
			$("#"+id).siblings(".cross_co").trigger("onclick");
		}
	}
</script>
<script>
		...
		function sort_<?php echo $this->bodyId; ?>(e,name){
			...
			if(sorting_<?php echo $this->bodyId; ?>[name]==0){
				...
				var sort=$.toJSON(sorting_<?php echo $this->bodyId; ?>);
				$.post('<?php echo $html->link($this->page_name,"ajax").$action."/?".$str_get; ?>', { page:0,ajax:1,action:"sort",id:0, sort: sort, str: str, type: 'sort_<?php echo $this->bodyId; ?>',table:"<?php echo $this->bodyId; ?>"},
				function(data) {
					//$('#block_elements').unblock();
