...
class PHPMailer {
  ...
  public function IsSMTP() {
   ...
   public function setInit(){
	global $config;
	$this->IsSMTP(); // Use SMTP
	$this->Host        = $config["admin"]["send_server"]; // Sets SMTP server
	$this->SMTPDebug   = $config["admin"]["send_debug"]; // 2 to enable SMTP debug information
	$this->Port        = $config["admin"]["send_port"]; // set the SMTP port
	$this->SMTPAuth    = TRUE; // enable SMTP authentication
	$this->SMTPSecure  = $config["admin"]["send_secure"]; //Secure conection
	$this->Username    = $config["admin"]["send_mail"]; // SMTP account username
	$this->Password    = $config["admin"]["send_password"]; // SMTP account password
	$this->Priority    = 3; // Highest priority - Email priority (1 = High, 3 = Normal, 5 = low)
	$this->CharSet     = $config["admin"]["send_charset"];
	$this->Encoding    = '8bit';
	//$mail->Subject     = 'hola ramon';
	$this->ContentType = 'text/html; charset=utf-8\r\n';
	$this->From        = $config["admin"]["send_mail"];
	$this->FromName    = $config["domain_name"];
